{{-- Hotel Rooms --}}
<?php $imageIndex = 0; ?>
@forelse ($hotelRoom as $hotelRoom_key => $hotelRoom_value)
	<?php 
		$imageCount = count($hotelRoom_value['Images']); 
		
		if($imageIndex == $imageCount-1){
			$imageIndex = 0;
		}

		$imageFile = ifset($hotelRoom_value['Images'][$imageIndex], urlDefaultImageRoom());
		$imageIndex++;
	?>
	{{-- Room Type --}}
		...
		<div class="col-md-12 col-sm-12 col-xs-12 main-roomContainer nopadding">
			...
			<div class="col-md-3 col-sm-3 col-xs-12 nopadding">
				<div class="col-md-11 col-sm-11 col-xs-12 nopadding height-120px">
					<img src="{{ $imageFile }}" alt="" height="100%" width="100%">
				</div>
			</div>
			{{-- /Room Type Image Div  --}}
				...
				<div class="row">
					<div class="col-md-12 col-sm-12 col-xs-12">
						<div class="m-top-10">
							...
							<div class="col-md-12 col-sm-12 col-xs-12 m-top-5">
								<div class="col-md-4 col-sm-4 col-xs-12">
									...
									<div id="priceBreakupBody{{ $requestIndex.'_'.$hotelRoom_key }}" hidden>
										@if (isset($hotelRoom_value['PriceBreakUp']))
	                      ...
	                      <tbody>
													@forelse($hotelRoom_value['PriceBreakUp'] as $priceBreakUp_key => $priceBreakUp)
														<tr>
					...
					<div class="col-md-12 col-sm-12 col-xs-12 pull-right">
						<div>
							<h3 class="text-center">
								...
								</i><span>{{ $hotelRoom_value['Price'] }}</span>
							</h3>
						</div>
						<div class="m-top-20">
							<button class="btn btn-block btn-primary btn-bookRoom" data-bookIndex="{{ $hotelRoom_key }}">Book</button>
						</div>
					</div>
				</div>
			</div>
