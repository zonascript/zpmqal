...
<script>
	$(document).on('click','.btn-chooseRoom', function(){
		...
		if(isVisible){
			...
			$.ajax({
				type:"post",
				url: "{{ urlHotelsRoomBuilder($urlVariable->id, $urlVariable->packageDbId, $urlVariable->packageHotelId) }}",
				data: data,
...
<script>
	$(document).on('click', '.btn-bookRoom', function(){
		...
		$.ajax({
			type:"post",
			url: "{{ urlHotelsRoomBookBuilder($urlVariable->id, $urlVariable->packageDbId, $urlVariable->packageHotelId) }}",
			data: data,
			success: function(responce, textStatus, xhr) {
