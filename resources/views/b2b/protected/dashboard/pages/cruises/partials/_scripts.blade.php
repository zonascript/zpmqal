...
<script>
	$(document).ready(function() {
		...
		@foreach ($package->cruiseRoutes as $cruiseRouteKey => $cruiseRoute)
			postFgfCruise({{$cruiseRoute->cruise->id}}, {{$cruiseRoute->id}});
		@endforeach
		...
		initFilter : function (targetList) {
			...
			@foreach ($package->cruiseRoutes as $cruiseRouteKey => $cruiseRoute)
				if (targetList == {{ $cruiseRoute->id }}) {
					this.hotel_{{ $cruiseRoute->id }} = new List("hotel_{{ $cruiseRoute->id }}_div", options);
				}
			@endforeach
		}
	}
...
<script>
	<?php 
		...
		$idObject = [];
		foreach ($package->cruiseRoutes as $cruiseRouteKey  => $cruiseRoute) {
			$idObject['hotel_'.$cruiseRoute->id] = [
					'did' => $cruiseRoute->cruise->id,
					'rid' => $cruiseRoute->id,
				];
			$idObject['hotel_'.$cruiseRoute->id]['next_did'] = 'NaN';
			$idObject['hotel_'.$cruiseRoute->id]['next_rid'] = 'NaN';

			if ($cruiseRouteKey+1 < $package->cruiseRoutes->count()) {
				$idObject['hotel_'.$cruiseRoute->id]['next_did'] = $package->cruiseRoutes[$cruiseRouteKey+1]->cruise->id;
				$idObject['hotel_'.$cruiseRoute->id]['next_rid'] = $package->cruiseRoutes[$cruiseRouteKey+1]->id;
			}
...
<script>
	$(document).on('keypress keyup keydown', "#filter_search", function(){
		...
		var search = $(this).val();
		@foreach ($package->cruiseRoutes as $cruiseRouteKey => $cruiseRoute)
			if (targetList == "hotel_{{ $cruiseRoute->id }}_div") {
				filter.hotel_{{ $cruiseRoute->id }}.search(search);
			}
		@endforeach
	});
