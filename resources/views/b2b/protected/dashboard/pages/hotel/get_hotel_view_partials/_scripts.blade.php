...
<script>
	$(document).on('click','.btn-chooseRoom', function(){
		...
		if(isVisible){
			...
			$.ajax({
				type:"post",
				url: "{{ urlHotelsRoomBuilder($packageHotel->id) }}",
				data: data,
...
<script>
	$(document).on('click', '.btn-bookRoom', function(){
		...
		$.ajax({
			type:"post",
			url: "{{ urlHotelsRoomBookBuilder($packageHotel->id) }}",
			data: data,
			success: function(responce, textStatus, xhr) {
...
@include('b2b.protected.dashboard.pages.hotel.get_hotel_view_partials.tbtq')

@include('b2b.protected.dashboard.pages.hotel.get_hotel_view_partials.skyscanner')
