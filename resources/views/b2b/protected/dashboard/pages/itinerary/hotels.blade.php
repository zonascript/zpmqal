							...
							<div class="x_title" >
								<div class="col-md-12 col-sm-12 col-xs-12">
									...
									<div class="col-md-5 col-sm-5 col-xs-12">
										<h2 class="pull-right">
											(
												{{date_formatter($packageInfo->start_date, 'Y-m-d', 'd-M-Y') }} 
													To 
												{{date_formatter($packageInfo->end_date, 'Y-m-d', 'd-M-Y') }} 
											)
										</h2>
									</div>
								</div>
							...
							<div class="x_content nopadding">
								<div class="form-group">
									...
									<div class="destinationClass">
										<div id="destination1" data-destination="1" class="col-md-12 col-sm-12 col-xs-12 form-group-self destinationList">
											...
											<div class="col-md-3 col-sm-3 col-xs-12">
												<select class="form-control nopadding p-left-10 selectNight" required="" data-parsley-type="integer" data-parsley-gt="0">
									...
									<div id="room">
										...
										<?php 
											$noOfRoom = (($room+1)/2); 
											$adultValue = isset($packageInfo->guests_detail[$noOfRoom-1]->NoOfAdults) 
																		? $packageInfo->guests_detail[$noOfRoom-1]->NoOfAdults
																		: 1;
											$childValue = isset($packageInfo->guests_detail[$noOfRoom-1]->ChildAge) 
																		? $packageInfo->guests_detail[$noOfRoom-1]->ChildAge
																		: [];
										?>
										{{-- /using some logic here --}}

										<div id="room_{{ ($room+1)/2 }}" {{ $room > 2*count($packageInfo->guests_detail) ? 'hidden' : '' }} class="roomGuest">
											<div class="col-md-12 col-sm-12 col-xs-12 p-bottom-1 m-bottom-n-5 form-group has-feedback">
												<label for="">Room : {{ $noOfRoom }}</label>
											</div>
												...
												<div class="center">
													<div class="input-group">
														...
														<span class="form-control text-center nopadding-right">
															<span id="a_value">
																<input type="text" name="quant_{{ $room }}" class="width-10 nostyle input-number noOfAdult" value="{{ $adultValue }}" min="1" max="4" disabled="disabled" required="" data-parsley-type="integer" data-parsley-gt="0">
															</span>
											...
											<div class="col-md-4 col-sm-4 col-xs-12 p-bottom-1 m-bottom-n-5 form-group has-feedback">
												<div class="center">
													<div class="input-group">
														...
														<span class="form-control text-center nopadding-right">
															<span id="a_value">
																<input type="text" name="quant_{{ $room }}" class="width-10 nostyle input-number noOfChild" value="{{ count($childValue) }}" min="0" max="2" disabled="disabled">
															</span>
											...
											{{-- Age html --}}
											<div class="col-md-4 col-sm-4 col-xs-12 p-bottom-1 m-bottom-n-5 form-group has-feedback age" data-age="quant_{{ $room }}">
												@foreach ($childValue as $childAge)
													<div class="col-md-6 col-sm-6 col-xs-12 p-bottom-1 form-group has-feedback nopadding">
														<select class="form-control nopadding age-elem" required="" data-parsley-type="integer" data-parsley-gt="0">
															...
															@for ($i = 1; $i <= 12; $i++)
															<option {{ $childAge == $i ? 'selected' : ''}}>{{ $i }}</option>
															@endfor
														</select>
													</div>
												@endforeach
									...
									<div class="col-md-12 col-sm-12 col-xs-12 p-bottom-1">
										<div >
											...
											<span id="pipeSapr" {{ count($packageInfo->guests_detail) == 1 ? 'hidden' : '' }}> | </span>
											<a id="btn-removeRoom" class="btn-link cursor-pointer" {{ count($packageInfo->guests_detail) == 1 ? 'hidden' : '' }}>Remove Room</a>
										</div>
									</div>
									{{-- /Add Room button --}}
